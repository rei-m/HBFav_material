...
class NewEntryPresenterTest {

    @Mock
    lateinit var getNewEntriesUsecase: GetNewEntriesUsecase

    @Mock
    lateinit var view: NewEntryContact.View

    lateinit var presenter: NewEntryPresenter
    ...
    fun setUp() {
        ...
        presenter = NewEntryPresenter(getNewEntriesUsecase)
    ...
    fun testOnResume_initialize_success() {
        ...
        `when`(getNewEntriesUsecase.get(EntryTypeFilter.ALL)).thenReturn(Observable.just(entryList))
    ...
    fun testOnResume_initialize_empty() {
        ...
        `when`(getNewEntriesUsecase.get(EntryTypeFilter.ALL)).thenReturn(Observable.just(arrayListOf()))
    ...
    fun testOnResume_initialize_failure() {
        ...
        `when`(getNewEntriesUsecase.get(EntryTypeFilter.ALL)).thenReturn(TestUtil.createApiErrorResponse(HttpURLConnection.HTTP_INTERNAL_ERROR))
    ...
    fun testOnResume_initialize_restart() {
        ...
        verify(getNewEntriesUsecase, timeout(TimeUnit.SECONDS.toMillis(1))).get(EntryTypeFilter.ALL)
    ...
    fun testOnRefreshList() {
        ...
        `when`(getNewEntriesUsecase.get(EntryTypeFilter.ALL)).thenReturn(Observable.just(finallyDisplayList))
    ...
    fun testOnOptionItemSelected() {
        ...
        `when`(getNewEntriesUsecase.get(EntryTypeFilter.ANIMATION_AND_GAME)).thenReturn(Observable.just(nextEntryList))
